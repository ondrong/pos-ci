...
//simpan header transaksi
	function _simpan_header(){
		...
		$.post('set_header_trans',{
			'no_trans'		:$('#no_transaksi').val(),
			'tanggal'		:$('#tgl_transaksi').val(),
			'faktur'		:'RK-'+$('#faktur_transaksi').val(),
	...
	function _simpan_pembayaran(){
		...
		$.post('simpan_bayar',{
			...
			_update_stock_ret($('#no_transaksi').val(),$('#tgl_transaksi').val());//print struk pembayaran
			keluar();
...
*/	function _update_stock_ret(id,tgl,jml){
		var path=$('#path').val();

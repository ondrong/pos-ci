...
$(document).ready(function(e) {
	...
	$('#no_doc')
		.focusout(function(){
			var str=$(this).val();
			$.post(path+'penjualan/get_transaksi',{'no_transaksi':str},
				function(result){
	...
	$('#nm_barang')
		.focus(function(){
				pos_div(this);
				auto_suggest2(path+'penjualan/get_material',$('#no_doc').val(),$(this).attr('id')+'-frm1');
		})
	...
	var path=$('#path').val();
		var nt=$('#no_doc').val();
			 $.post(path+'penjualan/get_detail_transaksi',{'no_transaksi':nt,'nm_barang':id},
					function(result){
						//alert(result);
	...
	unlock('input');
			...
			$.post('simpan_transaksi',
				...
				function(result){
					document.location.href=path+'pembelian/return_beli'
				})
	}